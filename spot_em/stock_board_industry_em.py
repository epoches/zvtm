...
def stock_board_block_stock_em_bycode(symbol: str = "BK0501",cate: str = 'industry') -> pd.DataFrame:
    url = "http://29.push2.eastmoney.com/api/qt/clist/get"
    params = {
        "pn": "1",
        "pz": "2000",
    ...
    temp_df['code'] = symbol
    temp_df['exchange'] = 'cn'
    temp_df['entity_type'] = 'block'
    temp_df["id"] = temp_df[["entity_type", "exchange", "code"]].apply(lambda x: "_".join(x.astype(str)), axis=1)
    temp_df["entity_id"] = temp_df["id"]
    df_block = get_block_lists_codes(cate,symbol)
    temp_df["name"] = df_block['name'].iloc[0]
    return temp_df

def stock_board_block_stock_em() -> pd.DataFrame:
    """
    东方财富网-沪深板块-行业板块-板块成份 保存到 em_block_meta 的 block_stock
    https://data.eastmoney.com/bkzj/BK1027.html
    """
    # 从 block 获取板块数据

    df_block_ind = get_block_lists('industry')
    df_block_conc = get_block_lists('concept')
    # 根据板块数据 获取板块股票数据
    df = pd.DataFrame()
    dfs = []
    for i in range(len(df_block_ind)):
        dfs.append(stock_board_block_stock_em_bycode(df_block_ind.loc[i,'code'],'industry'))
        df = pd.concat(dfs)
    for i in range(len(df_block_conc)):
        dfs.append(stock_board_block_stock_em_bycode(df_block_conc.loc[i,'code'],'concept'))
        df = pd.concat(dfs)
