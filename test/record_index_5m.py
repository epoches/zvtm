...
df0 = StockTradeDay.query_data(provider='joinquant',start_timestamp='2015-01-01',end_timestamp='2023-12-31')
for i in range(len(df0)):
    print(df0.iloc[i]['timestamp'])
    if i < len(df0)-1:
        Index5mKdata.record_data(provider="em", code='000001',start_timestamp = df0.iloc[i]['timestamp'].strftime("%Y-%m-%d"), end_timestamp = df0.iloc[i+1]['timestamp'].strftime("%Y-%m-%d"), day_data=False,sleeping_time=5)
